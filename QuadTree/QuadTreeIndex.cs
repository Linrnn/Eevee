...
namespace Eevee.QuadTree
{
    ...
    /// 四叉树坐标
    /// </summary>
    public readonly struct QuadTreeIndex : IEquatable<QuadTreeIndex>, IComparable<QuadTreeIndex>, IFormattable
    {
        #region 字段/初始化
        public static readonly QuadTreeIndex Invalid = new(-1, -1, -1); // 无效索引
        public static readonly QuadTreeIndex Root = new(0, 0, 0); // 根节点索引

        public readonly int Depth; // 所在层深度
        public readonly int X; // 所在层的X坐标
        public readonly int Y; // 所在层的Y坐标

        public QuadTreeIndex(int depth, int x, int y)
        {
            Depth = depth;
            X = x;
            Y = y;
        ...
        #region 基础方法
        public QuadTreeIndex Parent() => this == Root ? Invalid : new QuadTreeIndex(Depth - 1, X >> 1, Y >> 1);
        ...
        #region 运算符重载
        public static bool operator ==(in QuadTreeIndex lhs, in QuadTreeIndex rhs) => lhs.Depth == rhs.Depth && lhs.X == rhs.X && lhs.Y == rhs.Y;
        public static bool operator !=(in QuadTreeIndex lhs, in QuadTreeIndex rhs) => lhs.Depth != rhs.Depth || lhs.X != rhs.X || lhs.Y != rhs.Y;
        #endregion

        #region 继承/重载
        public override bool Equals(object obj) => obj is QuadTreeIndex other && this == other;
        public override int GetHashCode() => Depth ^ X ^ Y;
        public bool Equals(QuadTreeIndex other) => this == other;
        public int CompareTo(QuadTreeIndex other)
        {
