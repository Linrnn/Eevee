...
namespace Eevee.Fixed
{
    ...
    public readonly struct CircleInt : IEquatable<CircleInt>, IComparable<CircleInt>, IFormattable
    {
        ...
        public bool IsOutside(Vector2DInt point) => SqrDistance(point) > R * R; // 外离
        public bool IsIntersect(Vector2DInt point) => SqrDistance(point) == R * R; // 相切
        public bool IsContain(Vector2DInt point) => SqrDistance(point) < R * R; // 内含

        public bool Intersect(in CircleInt other) // 外切/相交/内切
        {
            int d = SqrDistance(in other);
        ...
        public bool Contain(in CircleInt other) // 内含/内离/同心
        {
        ...
        public bool IsOutside(in CircleInt other) // 外离
        {
            int d = SqrDistance(in other);
            int r = R + other.R;
            return d > r * r;
        }
        public bool IsCircumscribed(in CircleInt other) // 外切
        {
            int d = SqrDistance(in other);
            int r = R + other.R;
            return d == r * r;
        }
        public bool IsIntersect(in CircleInt other) // 相交
        {
            int d = SqrDistance(in other);
        ...
        public bool IsInscribed(in CircleInt other) // 内切
        {
            int d = SqrDistance(in other);
            int r = R - other.R;
            return d == r * r;
        }
        public bool IsContain(in CircleInt other) // 内含/内离
        {
            int d = SqrDistance(in other);
            int r = R - other.R;
            return d < r * r;
        }
        public bool IsConcentric(in CircleInt other) => X == other.X && Y == other.Y; // 同心
        #endregion
