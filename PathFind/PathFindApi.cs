...
namespace Eevee.PathFind
{
    ...
    public interface IPathFindObjectPoolGetter
    {
        ...
        void Release<T>(ref List<T> collection); // TryReleaseAndSetNull

        Stack<T> StackAlloc<T>();
        void Release<T>(Stack<T> collection);
    ...
    internal struct PathFindMtCs
    {
        ...
        internal readonly Dictionary<short, uint> AreaCount; // 区域编号，Key：区域编号，Value：区域数量
        ...
        internal PathFindMtCs(Vector2DInt16 size)
        {
            ...
            AreaCount = new Dictionary<short, uint>();
            AreaIdAllocator = 1;
        }
    }
