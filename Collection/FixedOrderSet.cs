...
namespace Eevee.Collection
{
    ...
    public sealed class FixedOrderSet<T> : ISet<T>, IReadOnlyCollection<T>, ISerializable, IDeserializationCallback
    {
        ...
        public FixedOrderSet(int capacity, IEqualityComparer<T> comparer)
        {
            ...
            if (capacity < 0)
                throw new ArgumentOutOfRangeException(nameof(capacity), $"初始化长度错误：{capacity}");
            if (capacity > 0)
                Initialize(capacity);
        ...
        {
            if (enumerator == null)
                throw new ArgumentNullException(nameof(enumerator), "初始化失败，enumerator不能为空！");
        ...
        public void Clear()
        {
            ...
            Assert.IsNotNull(_buckets, "_buckets is null!", string.Empty, string.Empty, string.Empty);
            for (int i = 0; i < _buckets.Length; i++)
                _buckets[i] = -1;
        ...
        {
            if (array == null)
                throw new ArgumentNullException(nameof(array), "目标数组不能为空");
            if (index < 0 || index > array.Length)
                throw new ArgumentOutOfRangeException(nameof(index), $"错误的位置编号：{index}【范围：[0, {array.Length})】");
            if (length < 0 || length > array.Length - index)
                throw new ArgumentException($"长度不足：{array.Length - index} Count：{Count}");
