    ...
    internal sealed class EditorDrawPathFindResult : MonoBehaviour
    {
        ...
        private void DrawPath()
        {
            ...
            foreach (int index in _indexes)
            {
                ...
                PathFindDraw.Text(start.X, start.Y, _gridSize, _minBoundary, _height, in _startColor, _drawPoint);

                for (int count = _points.Count - 1, i = 1; i < count; ++i)
                {
                    var point = _points[i];
                    PathFindDraw.Grid(point.X, point.Y, _gridSize, _minBoundary, _height, in _lineColor);
                    PathFindDraw.Text(point.X, point.Y, _gridSize, _minBoundary, _height, in _lineColor, _drawPoint);
                }

                var end = _points[^1];
                PathFindDraw.Grid(end.X, end.Y, _gridSize, _minBoundary, _height, in _endColor);
                PathFindDraw.Text(end.X, end.Y, _gridSize, _minBoundary, _height, in _endColor, _drawPoint);
                PathFindDraw.Arrow(end.X, end.Y, ((Vector2)(end - _points[^2])).normalized, _gridSize, _minBoundary, _height, in _endColor);

                for (int i = 1; i < _points.Count; ++i)
                    if (_points[i - 1] is { } p0 && _points[i] is { } p1)
                        PathFindDraw.Line(p0.X, p0.Y, p1.X, p1.Y, _gridSize, _minBoundary, _height, in _lineColor);

                if (_drawNext && _proxy.GetCurrentPoint(index) is { } currPoint && PathFindDiagnosis.GetNextPoint(_findFunc, index) is { } nextPoint)
                {
                    PathFindDraw.Grid(currPoint.x, currPoint.y, _gridSize, _minBoundary, _height, in _nextColor);
                    PathFindDraw.Line(currPoint.x, currPoint.y, nextPoint.X, nextPoint.Y, _gridSize, _minBoundary, _height, in _nextColor);
                    PathFindDraw.Arrow(nextPoint.X, nextPoint.Y, ((Vector2)(nextPoint - currPoint)).normalized, _gridSize, _minBoundary, _height, in _nextColor);
                }
            }
        }
    }
